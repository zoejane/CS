...
def crawl_web(seed,max_depth):
    ...
    crawled = []
    next_depth = []
    #a key list to store url's about depth
    depth = 0
    while tocrawl:
        page = tocrawl.pop()
        if page not in crawled and depth <= max_depth:
            union(next_depth, get_all_links(get_page(page)))
            #not store in tocrawl, but in next_depth
            crawled.append(page)
        if not tocrawl:
            tocrawl, next_depth = next_depth,[]
            #exchange made tocrawl become next_depth
            depth = depth + 1
    return crawled
...
#print crawl_web("http://top.contributors/velak.html",0)
